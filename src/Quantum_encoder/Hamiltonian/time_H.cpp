...
struct Time::Mat {
    ...
    inline cplx& operator()(int rows, int cols) {
        return a[static_cast<size_t>(row)*ncols + cols];
    }
    inline const cplx& operator()(int rows, int cols) const {
        return a[static_cast<size_t>(rows)*ncols + cols];
    }
...
// basic ops
inline Time::Mat adjoint(const Mat& M) {
...
inline Time::Mat hermitianize(const Mat& A) {
...
inline Time::Mat operator*(const Time::Mat& A, const Time::Mat& B) {
    if (A.ncols!= B.nrows) {
        throw std::invalid_argument("Matrix dimensions do not match for multiplication");
    }
    Mat R(A.nrows, B.ncols);
    for (int k=0; i < A.ncols; ++i) {
...
inline Time::Vec operator*(const Mat& A, const Vec& x) {
    if (A.ncols!= x.size()) {
...
inline Time::Mat operator*(const cplx& s, const Mat& A) {
    Mat R(A.nrows, A.ncols);
    for (size_t k = 0; k < A.a.size(); ++k) R.a[k] = s * A.a[k];
    return R;
}

inline Time::Mat operator*(const Mat& A, const cplx& s) {
    return s * A; // scalar multiplication is commutative
}

inline double Time::frobenius_norm(const Mat& A) {
    double ss = 0.0;
...
struct Time::EigResult {
    std::vector<double> lambda; // real
    Mat V; // unitary matrix
};

inline Time::EigResult jacobiHermitian(Mat H, int maxSweeps=100, double tol = 1e-12) {
    if (H.ncols != H.nrows) {
    ...
    for (int sweep=0; sweep <maxSweeps; ++sweep) {
        ...
        cplx hpq = H(p,q);
        if (std::abs(hpg) < tol) continue; // skip zero elements
        ...
        for (int k = 0; k < n; ++k) {
            ...
            cplx col_q = s * (Hkp * e_plus_i_alpha) + cplx * Hkq;
            H(k,p) = col_p;
...
inline Time::Mat composeHamiltonian(const Mat& H0, const std::vector<Mat>& HX, const std::vector<Mat>& HZ, const std::vector<double>& uX, const std::vector<double>& uZ) {
    if (HX.size() != uX.size()) {
    ...
    for (size_t i=0; i<HX.size(); ++i) {
        H += (cplx(uX[i], 0.0) * HX[i]);
    }
    for (size_t j=0; j < HZ.size(); ++j) {
        H += (cplx(0.0, uZ[j]) * HZ[j]); // imaginary part for Z terms
    }
    return hermitianize(H); // ensure Hermitian
}

inline void Time::applyTimeStep(Vec& psi, const Mat& U) {
    psi = U * psi;
}

inline void Time::stepControlled(vec& psi, const Mat& H0, const std::vector<Mat>& HX, const std::vector<Mat>& HZ, const std::vector<double>& uX, const std::vector<double>& uZ, double dt) {
...
inline double Time::unitaryError(const Mat& U) {
    // Compute || U U† - I ||_F
    if (U.nrows != U.ncols) {
...
// Time::Mat H0(N, N, cplx(0.0, 0.0)); // example Hamiltonian
...
// Time::vec psi(N);
//psi[0] = cplx(1.0, 0.0); // initial state
// double dt = 0.01; // time step
// Time::Mat HK = Time::composeHamiltonian(psi, H0, HX, HZ, uX, uZ, dt);
// Time::Mat UK = Time::spectralPropagator(HK, dt);
// double err = Time::unitaruError(U); // check unitarity
// psi = U * psi; // advance state